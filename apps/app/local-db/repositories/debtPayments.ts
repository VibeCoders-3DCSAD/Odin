...
export type DebtPayment = {
  ...
  amount_centavos: number; principal_centavos: number; interest_centavos: number; notes: string | null; version: number;
};
...
export async function listDebtPayments(userId: string, debtAccountId: string): Promise<DebtPayment[]> {
  ...
  return db.getAllAsync<DebtPayment>("SELECT id, debt_account_id, transaction_id, payment_date, amount_centavos, principal_centavos, interest_centavos, notes, version FROM debt_payments WHERE user_id = ? AND debt_account_id = ? AND deleted = 0 ORDER BY payment_date DESC, created_at DESC", userId, debtAccountId);
}

export async function listAllDebtPayments(userId: string): Promise<DebtPayment[]> {
  const db = await getDb();
  return db.getAllAsync<DebtPayment>("SELECT id, debt_account_id, transaction_id, payment_date, amount_centavos, principal_centavos, interest_centavos, notes, version FROM debt_payments WHERE user_id = ? AND deleted = 0 ORDER BY payment_date ASC, created_at ASC", userId);
}

type TransactionDebtPaymentInput = Pick<CreateExpenseInput, "amount_centavos" | "source_account_id" | "subcategory_id" | "transaction_date" | "merchant_name" | "notes"> & { debt_account_id: string; principal_centavos: number; interest_centavos: number };
type UpdateTransactionDebtPaymentInput = Omit<TransactionDebtPaymentInput, "debt_account_id">;

function validateAllocation(input: Pick<TransactionDebtPaymentInput, "amount_centavos" | "principal_centavos" | "interest_centavos">) {
  ...
  if (!Number.isSafeInteger(input.principal_centavos) || input.principal_centavos < 0 || !Number.isSafeInteger(input.interest_centavos) || input.interest_centavos < 0 || input.principal_centavos + input.interest_centavos > input.amount_centavos) throw new LocalDbError("VALIDATION_ERROR", "Principal and interest or fees must be non-negative and cannot exceed the payment amount.");
}

export async function createTransactionDebtPayment(userId: string, deviceId: string, input: TransactionDebtPaymentInput): Promise<void> {
  ...
  await db.withTransactionAsync(async () => {
    ...
    if (input.principal_centavos > debt.current_balance_centavos) throw new LocalDbError("VALIDATION_ERROR", "Principal amount cannot exceed the current debt balance.");
...
export async function deleteTransactionDebtPayment(userId: string, deviceId: string, paymentId: string): Promise<void> {
  ...
  await db.withTransactionAsync(async () => {
    const payment = await db.getFirstAsync<DebtPayment>("SELECT id, debt_account_id, transaction_id, payment_date, amount_centavos, principal_centavos, interest_centavos, notes, version FROM debt_payments WHERE id = ? AND user_id = ? AND deleted = 0", paymentId, userId);
    ...
    await db.runAsync("UPDATE debt_accounts SET current_balance_centavos = current_balance_centavos + ?, status = 'active', paid_off_at = NULL, version = version + 1, updated_at = ? WHERE id = ? AND user_id = ?", payment.principal_centavos, ts, payment.debt_account_id, userId);
...
export async function updateTransactionDebtPayment(userId: string, deviceId: string, paymentId: string, input: UpdateTransactionDebtPaymentInput): Promise<void> {
  validateAllocation(input);
  const db = await getDb();
  const ts = new Date().toISOString();
  await db.withTransactionAsync(async () => {
    ...
    if (!debt || input.principal_centavos > debt.current_balance_centavos + payment.principal_centavos) throw new LocalDbError("VALIDATION_ERROR", "Principal amount cannot exceed the current debt balance.");
    ...
    const currentBalance = debt.current_balance_centavos + payment.principal_centavos - input.principal_centavos;
    await db.runAsync("UPDATE debt_payments SET amount_centavos = ?, principal_centavos = ?, interest_centavos = ?, payment_date = ?, notes = ?, version = version + 1, updated_at = ? WHERE id = ? AND user_id = ?", input.amount_centavos, input.principal_centavos, input.interest_centavos, input.transaction_date, input.notes ?? null, ts, paymentId, userId);
    ...
    await enqueueOperation(db, { userId, deviceId, entity: "debt_payments", recordId: payment.id, operationType: "update", baseVersion: payment.version, changedFields: ["amount_centavos", "principal_centavos", "interest_centavos", "payment_date", "notes"], payload: { debt_account_id: payment.debt_account_id, transaction_id: payment.transaction_id, source: "transaction", payment_date: input.transaction_date, amount_centavos: input.amount_centavos, principal_centavos: input.principal_centavos, interest_centavos: input.interest_centavos, notes: input.notes ?? null, linked_transaction_type: "expense", linked_source_account_id: input.source_account_id, linked_subcategory_id: input.subcategory_id }, failureMessage: "This debt payment could not be updated." });
  });
}
