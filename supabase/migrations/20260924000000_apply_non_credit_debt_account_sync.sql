-- The legacy debt wrapper delegates account operations through a renamed
-- function that can resolve to the generic entity allowlist. Handle the
-- account CRUD path directly and retain the hardened handler for other debt
-- entities.
  ...
  ELSE
    ...
    ELSE
      UPDATE debt_accounts SET
        linked_account_id = CASE WHEN p_payload ? 'linked_account_id' THEN NULLIF(p_payload->>'linked_account_id', '')::uuid ELSE linked_account_id END,
        ...
        archived_at = CASE WHEN p_payload->>'status' = 'archived' THEN COALESCE(archived_at, now()) WHEN p_payload ? 'status' THEN NULL ELSE archived_at END,
