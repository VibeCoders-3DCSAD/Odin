...
export async function requestCreditCardSettlement(userId: string, deviceId: string, input: { installmentId: string; settlementDate: string; remainingPrincipalCentavos: number; settlementAmountCentavos: number; preterminationFeeCentavos?: number }): Promise<string> {
