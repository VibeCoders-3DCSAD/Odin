-- Persist the schedule configuration already carried by the offline schema.
-- The account-specific wrapper intentionally owns debt_accounts operations.
...
BEGIN
  ...
  IF p_payload ? 'payment_schedule' AND p_payload->'payment_schedule' ? 'customIntervalDays' AND COALESCE(p_payload->'payment_schedule'->>'customIntervalDays', '') !~ '^[1-9][0-9]*$' THEN RAISE EXCEPTION 'custom payment interval is invalid'; END IF;
  IF p_payload ? 'payment_schedule' AND p_payload->'payment_schedule' ? 'semiMonthlyDays' AND (jsonb_typeof(p_payload->'payment_schedule'->'semiMonthlyDays') <> 'array' OR jsonb_array_length(p_payload->'payment_schedule'->'semiMonthlyDays') <> 2 OR EXISTS (SELECT 1 FROM jsonb_array_elements_text(p_payload->'payment_schedule'->'semiMonthlyDays') AS day WHERE day !~ '^(?:[1-9]|1[0-9]|2[0-8])$') OR p_payload->'payment_schedule'->'semiMonthlyDays'->>0 = p_payload->'payment_schedule'->'semiMonthlyDays'->>1) THEN RAISE EXCEPTION 'semi-monthly payment days are invalid'; END IF;
  IF p_payload->>'payment_frequency' = 'custom' AND COALESCE(p_payload->'payment_schedule'->>'customIntervalDays', '') !~ '^[1-9][0-9]*$' THEN RAISE EXCEPTION 'custom payment interval is required'; END IF;
  ...
  IF p_operation_type = 'create' THEN
    ...
    INSERT INTO debt_accounts (id, user_id, linked_account_id, name, lender_name, preset_key, status, original_balance_centavos, current_balance_centavos, annual_interest_rate_bps, minimum_payment_centavos, payment_frequency, payment_schedule, next_due_date, maturity_date, target_payoff_date, interest_period, interest_method, preset_data, notes, version, deleted, updated_at)
    VALUES (p_record_id, v_user_id, NULLIF(p_payload->>'linked_account_id', '')::uuid, p_payload->>'name', p_payload->>'lender_name', p_payload->>'preset_key', 'active', COALESCE((p_payload->>'original_balance_centavos')::bigint, 0), COALESCE((p_payload->>'current_balance_centavos')::bigint, 0), COALESCE((p_payload->>'annual_interest_rate_bps')::integer, 0), COALESCE((p_payload->>'minimum_payment_centavos')::bigint, 0), COALESCE(p_payload->>'payment_frequency', 'monthly'), COALESCE(p_payload->'payment_schedule', '{}'::jsonb), NULLIF(p_payload->>'next_due_date', '')::date, NULLIF(p_payload->>'maturity_date', '')::date, NULLIF(p_payload->>'target_payoff_date', '')::date, p_payload->>'interest_period', p_payload->>'interest_method', COALESCE(p_payload->'preset_data', '{}'::jsonb), p_payload->>'notes', 1, false, now());
    v_version := 1;
  ELSE
