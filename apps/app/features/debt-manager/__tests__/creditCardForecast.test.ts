import { buildCreditCardForecast, buildCreditCardForecastAsync } from "../creditCardForecast";
...
describe("buildCreditCardForecast", () => {
  ...
  it("shows amortization payments after the first statement payment", () => {
    ...
    expect(forecast.points.slice(3, 5)).toMatchObject([
      { regularPaymentCentavos: 3_000_000, amortizationPaymentCentavos: 250_000, targetCentavos: 3_250_000 },
  ...
  it.each([
    ["pay_in_full", null, null, [0]],
    ["pay_minimum", null, null, [2_500_000, 2_000_000, 1_500_000]],
    ["percentage_of_statement", null, 5_000, [1_500_000, 0]],
    ["custom_payment", 1_000_000, null, [2_000_000, 1_000_000, 0]],
  ] as const)("reuses the %s strategy on succeeding nodes", (strategyName, customAmountCentavos, percentageBps, expectedBalances) => {
    const forecast = buildCreditCardForecast({
      cycles: [{ id: "cycle-1", user_id: "user-1", account_id: "card-1", cycle_start_date: "2026-08-05", cutoff_date: "2026-09-04", statement_date: "2026-09-04", version: 1, deleted: false, created_at: "2026-08-05T00:00:00.000Z", updated_at: "2026-08-05T00:00:00.000Z" }],
      transactions: [],
    ...
    if (strategyName === "pay_in_full") {
      expect(forecast.points.slice(0, 2).map((point) => 10_000_000 - point.availableCreditCentavos)).toEqual([3_000_000, 0]);
    }
    const forecastPoints = forecast.points.filter((point) => point.cycleId.startsWith("forecast"));
    expect(forecastPoints.every((point) => point.date >= "2026-09-20")).toBe(true);
    expect(forecastPoints.slice(0, expectedBalances.length).map((point) => 10_000_000 - point.availableCreditCentavos)).toEqual(expectedBalances);
  });
});
