-- Restore the recovery-safe transaction reconciliation from migration 08.
-- Migration 12 reintroduced a global overdraft rejection, which prevented
-- deleting a payment when any historical account had a negative balance.
...
BEGIN
  ...
  IF p_entity = 'savings_goals' THEN
    RETURN QUERY SELECT * FROM private.apply_savings_goal_sync_operation_core(
