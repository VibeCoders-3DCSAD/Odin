-- A soft delete must converge even after one of its historical references was
-- removed. Relationship validation still applies to every live write.
CREATE OR REPLACE FUNCTION public.assert_credit_card_relationships()
RETURNS trigger
LANGUAGE plpgsql
AS $$
DECLARE
