-- Installments are queued after their base transaction and before the credit-card
-- relationship. The relationship is the invariant that binds all three records,
-- so the installment itself must not require that relationship to exist first.
CREATE OR REPLACE FUNCTION public.apply_credit_card_sync_operation_v3(
