ALTER TABLE budgets DROP CONSTRAINT IF EXISTS budgets_period_chk;
ALTER TABLE budgets DROP CONSTRAINT IF EXISTS budgets_period_days_chk;
ALTER TABLE budgets
...
SET user_id = budgets.user_id
FROM budgets
WHERE allocations.budget_id = budgets.id
  AND allocations.user_id IS NULL;
...
ALTER TABLE budgets
  ADD CONSTRAINT budgets_period_chk CHECK (period_start <= period_end),
  ADD CONSTRAINT budgets_period_days_chk CHECK (
    budget_period_days > 0
    AND budget_period_days = (period_end - period_start) + 1
  );
...
BEGIN
  ...
  ELSE
    ...
    IF NOT v_deleted THEN
      UPDATE budgets SET status = 'deleted', deleted_at = now(), updated_at = now(), version = version + 1
