...
BEGIN
  ...
  IF p_payload ? 'totalAmountMinor' AND p_payload ? 'debtBudgetAmountMinor' AND (p_payload->>'debtBudgetAmountMinor')::numeric > (p_payload->>'totalAmountMinor')::numeric THEN RAISE EXCEPTION 'debt budget cannot exceed total'; END IF;
  ...
  IF p_payload ? 'totalAmountMinor' AND v_total + COALESCE((p_payload->>'debtBudgetAmountMinor')::numeric, 0) > (p_payload->>'totalAmountMinor')::numeric THEN RAISE EXCEPTION 'allocations and debt budget cannot exceed total'; END IF;
  ...
  IF v_result.status = 'applied' AND p_operation_type IN ('create', 'update') THEN
    UPDATE budgets SET debt_budget_amount_centavos = COALESCE((p_payload->>'debtBudgetAmountMinor')::bigint, debt_budget_amount_centavos), updated_at = now()
