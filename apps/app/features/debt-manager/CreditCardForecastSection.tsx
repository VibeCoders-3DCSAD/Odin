...
export default function CreditCardForecastSection({ forecast, isLoading, creditLimitCentavos }: Props) {
  ...
  const balancePoints = visiblePoints.map((point) => ({ id: `${point.cycleId}-${point.date}`, date: point.date, balanceCentavos: Math.max(0, creditLimitCentavos - point.availableCreditCentavos), isForecast: point.cycleId.startsWith("forecast") }));
