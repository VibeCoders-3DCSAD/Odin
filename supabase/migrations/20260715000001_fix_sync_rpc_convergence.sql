    ...
    IF p_entity = 'categories' THEN
      ...
      IF EXISTS (
        ...
        WHERE user_id = v_user_id
          AND lower(label) = lower(p_payload->>'label')
          AND category_group_id = (p_payload->>'category_group_id')::uuid
          AND deleted = false
      ) THEN
        UPDATE applied_operations
        SET result = jsonb_build_object(
        ...
        RETURN QUERY SELECT 'rejected'::text, 'label already exists'::text, NULL::integer, NULL::text[];
        RETURN;
      END IF;
