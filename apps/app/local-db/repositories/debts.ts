...
export async function createDebtPaymentExpense(userId: string, deviceId: string, debtAccountId: string, input: { amountMinor: number; sourceAccountId: string; paymentDate: string; subcategoryId: string; notes?: string }) {
  ...
  await db.withTransactionAsync(async () => {
    ...
    await enqueueOperation(db, { userId, deviceId, entity: "debt_payments", recordId: paymentId, operationType: "create", baseVersion: null, changedFields: [], payload: { id: paymentId, user_id: userId, debt_account_id: debtAccountId, transaction_id: transaction.transaction.id, source: "transaction", payment_date: input.paymentDate, amount_centavos: input.amountMinor, principal_centavos: input.amountMinor }, failureMessage: "This debt payment could not be synced." });
    await enqueueOperation(db, { userId, deviceId, entity: "debt_accounts", recordId: debtAccountId, operationType: "update", baseVersion: debt.version, changedFields: ["current_balance_centavos", "status"], payload: { current_balance_centavos: debt.currentBalanceMinor - input.amountMinor, status: debt.currentBalanceMinor === input.amountMinor ? "paid_off" : debt.status }, failureMessage: "This debt balance could not be synced." });
  });
