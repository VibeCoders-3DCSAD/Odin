...
Extend the existing new-expense flow so a user who selects a credit-card account can choose an installment purchase, including an installment bought in an earlier cycle but recorded today. Atomically save the ledger expense, billing-cycle relationship, and long-term installment record. Debit available credit by the full original principal locally, while only one remaining monthly amortization represents the applicable billing-cycle amount, subject to later issuer reconciliation.
...
- No credit-card payment, credit-balance application, statement repayment-strategy, or non-credit-card debt workflow.
- Early settlement remains a separate follow-up flow: creating an installment always starts it as `active`; this slice must not create or imply an early-settlement request.
...
- Treat original principal as the purchase amount; for a new purchase, default remaining principal to that amount and remaining months to the term. Permit either remaining value to be lower only to support a historical/backfilled installment. Do not ask the user to manually re-enter original principal.
- Do not derive an issuer payment schedule from principal and interest. `monthly_amortization_centavos` is the issuer-provided or user-confirmed monthly due estimate; interest-bearing amortization can exceed principal reduction, so principal must not be used to cap a final installment amount.
- Add a pure cycle-amortization projection helper. For a new purchase, its linked cycle is the first due cycle. For a historical entry, skip `term_months - remaining_months` elapsed cycles after the linked purchase cycle and make the first remaining due land in the next applicable cycle. Project exactly `remaining_months` materialized card cycles, one monthly amortization each; never replay already-paid months or treat the original purchase amount as a cycle charge.
- Keep the installment lifecycle canonical across local DB, API, and Supabase: `active`, `early_settlement_requested`, and `completed`. Only a recognized settlement or issuer-recognized final payment may set `completed`.
...
- Replace the server insert invariant that requires remaining principal/months to equal their originals. It must instead enforce their bounds, so a backfilled installment can represent payments already made. Align the server status enum with the canonical lifecycle values.
...
- When the expense source is a credit card, expose the required `Regular Purchase / Installment Purchase` selector. For an installment, display original principal as the transaction amount (read-only), collect merchant/description, term, remaining principal, remaining months, monthly amortization, interest type, and interest rate only when interest-bearing.
- When amount changes, default remaining principal to the amount unless the user has already overridden it. When term changes, default remaining months to the term unless overridden. Explain that those two fields should be changed only when recording a historical installment with earlier payments already made.
- Set `settlement_status` to `active` internally. Do not expose an early-settlement control in this form: an early-settlement request requires its own date, settlement amount, fee, and issuer-recognition lifecycle.
...
- Use the cycle-amortization projection to show one monthly amortization in each remaining applicable cycle—not the entire original principal, a duplicated amount in every cycle, or already-paid cycles from a historical entry. Keep card available credit separately displayed as reduced by the issuer-recognized full hold.
...
- Add a backfill regression: a 12-month installment recorded after five paid months persists its original values, projects only seven future monthly dues beginning with the next applicable cycle, and passes local and remote validation. Add status-contract tests proving all layers accept the same lifecycle values and that creation defaults to `active`.
...
- A user selecting a credit-card expense source can explicitly choose Regular Purchase or Installment Purchase, including an installment bought in an earlier cycle.
...
- Available credit is reduced by the full original principal, while only each remaining applicable cycle represents one monthly amortization due.
...
- Debt Manager shows active installment details and does not mark an installment completed absent issuer-recognized settlement or final payment.
- A fresh purchase defaults remaining principal/months to the original principal/term; a historical entry can override them within validated bounds without being rejected by sync.
- Early settlement cannot be selected from a new-purchase form; it is recorded through its dedicated settlement flow.
