-- Credit cards are financial accounts. credit_card_details is the single
-- source for credit card specifics, so financial_accounts.credit_limit_centavos
-- is dropped and the sync RPC is rewritten to stop reading/writing the column.
-- The 4.2 account-form fields are added to credit_card_details with 4.6
-- validation enforced at the schema boundary too.

ALTER TABLE financial_accounts DROP COLUMN credit_limit_centavos;

ALTER TABLE credit_card_details
  ADD COLUMN billing_cycle_days integer CHECK (billing_cycle_days BETWEEN 28 AND 31),
  ADD COLUMN alert_threshold_percent integer CHECK (alert_threshold_percent BETWEEN 0 AND 100);

-- Rewrite of apply_sync_operation (20260717022002 shape) without the dropped column.
...
BEGIN
  ...
  IF p_operation_type = 'create' THEN
    ...
    ELSIF p_entity = 'financial_accounts' THEN
      INSERT INTO financial_accounts (id, user_id, name, kind, status, opening_balance_centavos, current_balance_centavos, include_in_dashboard_balance, institution_name, opened_on, sort_order, metadata, updated_at, version, deleted)
