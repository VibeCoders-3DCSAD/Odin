-- Keep issuer reconciliation authoritative for available credit while enforcing
-- the card's persisted credit-limit ceiling on every direct sync request.
...
BEGIN
  ...
  IF p_entity = 'credit_card_details'
     ...
     AND p_changed_fields <@ ARRAY['available_credit_centavos']
     AND 'available_credit_centavos' = ANY(p_changed_fields)
     AND p_payload ? 'available_credit_centavos'
     AND p_payload - 'available_credit_centavos' = '{}'::jsonb THEN
    ...
    SELECT version, credit_limit_centavos
      INTO v_version, v_current_credit_limit
    ...
    IF v_available_credit > v_current_credit_limit THEN
      RAISE EXCEPTION 'available credit cannot exceed the credit limit';
    END IF;
