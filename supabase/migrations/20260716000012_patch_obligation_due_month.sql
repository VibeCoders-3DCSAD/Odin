...
BEGIN
  ...
  ELSIF p_entity = 'financial_accounts' THEN
    ...
    v_overwritten_values :=
      ...
      CASE WHEN p_payload ? 'notes' THEN jsonb_build_object('notes', (SELECT to_jsonb(fa.notes) FROM financial_accounts fa WHERE fa.id = p_record_id AND fa.user_id = v_user_id)) ELSE '{}'::jsonb END;
  ...
  ELSIF p_entity = 'income_sources' THEN
    ...
    v_overwritten_values :=
      ...
      CASE WHEN p_payload ? 'payday_second_day_of_week' THEN jsonb_build_object('payday_second_day_of_week', (SELECT to_jsonb(payday_second_day_of_week) FROM income_sources WHERE id = p_record_id AND user_id = v_user_id)) ELSE '{}'::jsonb END ||
      ...
      CASE WHEN p_payload ? 'estimated_interval_days' THEN jsonb_build_object('estimated_interval_days', (SELECT to_jsonb(estimated_interval_days) FROM income_sources WHERE id = p_record_id AND user_id = v_user_id)) ELSE '{}'::jsonb END ||
  ...
  ELSIF p_entity = 'financial_obligations' THEN
    ...
    v_overwritten_values :=
      ...
      CASE WHEN p_payload ? 'due_second_day_of_month' THEN jsonb_build_object('due_second_day_of_month', (SELECT to_jsonb(due_second_day_of_month) FROM financial_obligations WHERE id = p_record_id AND user_id = v_user_id)) ELSE '{}'::jsonb END ||
      CASE WHEN p_payload ? 'due_day_of_week' THEN jsonb_build_object('due_day_of_week', (SELECT to_jsonb(due_day_of_week) FROM financial_obligations WHERE id = p_record_id AND user_id = v_user_id)) ELSE '{}'::jsonb END ||
      CASE WHEN p_payload ? 'due_second_day_of_week' THEN jsonb_build_object('due_second_day_of_week', (SELECT to_jsonb(due_second_day_of_week) FROM financial_obligations WHERE id = p_record_id AND user_id = v_user_id)) ELSE '{}'::jsonb END ||
      CASE WHEN p_payload ? 'due_month' THEN jsonb_build_object('due_month', (SELECT to_jsonb(due_month) FROM financial_obligations WHERE id = p_record_id AND user_id = v_user_id)) ELSE '{}'::jsonb END ||
