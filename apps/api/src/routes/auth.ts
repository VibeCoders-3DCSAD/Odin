...
async function cancelActiveDeletionRequests(
  ...
  for (let attempt = 1; attempt <= MAX_CANCEL_RETRIES; attempt++) {
    ...
    console.error("Failed to cancel active deletion requests", { user_id: userId, attempt, timestamp: new Date().toLocaleString("en-PH", { timeZone: "Asia/Manila" }), code: error.code, message: error.message });
