-- This is the complete, source-controlled successor to the historical
-- pg_get_functiondef/replace patch chain.
CREATE OR REPLACE FUNCTION public.apply_credit_card_sync_operation_core(
...
BEGIN
  ...
  IF v_payload ? 'settlement_status' AND v_payload->>'settlement_status' NOT IN ('active','early_settlement_requested','completed') THEN RAISE EXCEPTION 'settlement_status is invalid'; END IF;
...
REVOKE EXECUTE ON FUNCTION public.apply_credit_card_sync_operation_core(uuid, text, text, uuid, text, integer, text[], jsonb)
